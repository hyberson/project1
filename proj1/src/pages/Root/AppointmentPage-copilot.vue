...
<template v-for="(propDateAndTimes, index) in availableSlots" :key="index">
  ...
  <q-card-section>
    <!-- The date -->
    <q-card-section>
      <div class="text-h5">{{ (propDateAndTimes as {date: Date}).date }}</div>
...
const availableSlots: { propDateAndTimes: { theDate: Date, theTimes: string[] } }[] = [];
...
watch(() => shiftSelection, (newVal) => {
  ...
  */
  ...
  watch(() => shiftSelection, (newVal) => {
    ...
    const propDateAndTimes = availableSlots[0].propDateAndTimes

    const firstGroup = propDateAndTimes.theTimes.filter((time: string) => time <= lastOfTheMorning)

    const secondGroup = propDateAndTimes.theTimes.filter((time: string) => time > lastOfTheMorning)
  ...
  const secondGroup = propDateAndTimes.propDateAndTimes.theTimes.filter(time => time > lastOfTheMorning)

  const handleChipClick = (time: string) => {
    const selectedTimeSlot = `${propDateAndTimes.propDateAndTimes.theDate} Time: ${time}`
    console.log(selectedTimeSlot)
  }
